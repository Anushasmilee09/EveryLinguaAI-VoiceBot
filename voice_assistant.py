    ...
    def warm_up_microphone(self, warm_up_cycles=4):
        ...
        with sr.Microphone() as source:
            for _ in range(warm_up_cycles):
                self.recognizer.adjust_for_ambient_noise(source, duration=1.0)  # Adjusting for ambient noise each time
                try:
        ...
        while True:
            with sr.Microphone() as source:
                ...
                try:
                    ...
                    # Recognize directly in selected locale; avoid English pivot to keep full multilingual fidelity
                    text = self.recognizer.recognize_google(audio, language=language_code[0])
                    print("after recognizing")
                    # Stop / language change checks operate on localized text as well
                    if text.lower() in stop_phrases:
                        print("Thank you. Have a great day!")
                        break
                    elif any(phrase in text.lower() for phrase in change_language_phrases):
                        ...
                        print(f"[DEBUG] You said in {language_code[0]}: {text}")
                        
                        # For structured dealership logic that expects English, translate input to English only for logic,
                        # but DO NOT use that translation for user-facing response.
                        user_input_english = GoogleTranslator(source='auto', target='en').translate(text)
                        print(f"[DEBUG] Translated to English for logic: {user_input_english}")
                        
                        dealership_response = self.get_dealership_response(user_input_english)
                        print(f"[DEBUG] Dealership response (English): {dealership_response[:100]}...")

                        # Use Gemini to produce the final answer strictly in selected locale
                        # Include dealership context if logic returned generic guidance
                        if "I can help you with" in dealership_response or "Please let me know" in dealership_response:
                            print(f"[DEBUG] Using direct Gemini response in {language_code[0]}")
                            response = self.openai_client.chat_with_gemini(text, locale=language_code[0])
                        else:
                            # Provide dealership_response as context to Gemini and ask to answer in selected locale
                            print(f"[DEBUG] Using Gemini to translate dealership response to {language_code[0]}")
                            prompt = f"Translate and respond to this in a natural, conversational way: {dealership_response}"
                            response = self.openai_client.chat_with_gemini(prompt, locale=language_code[0])

                        print(f"[DEBUG] Final response in {language_code[0]}: {response[:100]}...")
                        
                        # Speak strictly in selected locale (e.g., ta-IN, hi-IN)
                        print(f"[DEBUG] Converting to speech in {language_code[0]}")
                        audio_filepath = self.openai_client.text_to_speech(response, language_code=language_code[0])
                        
                        if audio_filepath:
                            print(f"[DEBUG] Playing audio from: {audio_filepath}")
                            play_audio(audio_filepath)
                        else:
                            print(f"[ERROR] Failed to generate audio for {language_code[0]}")
